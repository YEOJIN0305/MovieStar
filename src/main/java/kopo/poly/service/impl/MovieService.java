...
//    @Scheduled(cron = "5 * * * * *")
    public int collectMovieRank() throws Exception {
        ...
        while (movie_rank.hasNext()) {
            ...
            log.info(this.getClass().getName() + ".collect_time!");
            ...
            log.info(this.getClass().getName() + ".setmovie_nm Start!");

            //영화 예매율
            pDTO.setMovie_reserve(CmmUtil.nvl(movie_reserve.next().text()).trim());
            log.info(this.getClass().getName() + ".reserve Start!");

            //영화 점수
            pDTO.setScore(CmmUtil.nvl(score.next().text()).trim());
            log.info(this.getClass().getName() + ".score Start!");
            ...
            log.info(this.getClass().getName() + ".openday Start!");

            //등록자
            pDTO.setReg_id("admin");
            log.info(this.getClass().getName() + ".reg_id Start!");
